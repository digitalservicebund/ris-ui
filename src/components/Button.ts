...
export default {
    root: ({ props, context, instance }: { props: ButtonProps; context: ButtonContext; instance: ButtonInstance }) => {
        ...
        if (instance.hasIcon && (!props.size || props.size === 'small') && props.label) {
            if (props.iconPos === 'left') {
        ...
        // Size adjustments
        if (props.size === 'large') {
            sizeStyling = props.label ? 'py-20 px-24 text-[1.125rem] min-h-64' : 'w-64 h-64';
        } else {
        ...
        if (props.severity === 'secondary') {
            if (!context.disabled) {
                secondarySeverityStyling = 'bg-white text-blue-800 border-solid border-blue-800 hover:bg-gray-200 active:bg-white active:text-blue-800 active:border-transparent focus:outline-none focus:ring-blue-800 focus:ring-4 focus:ring-offset-2 focus:bg-gray-200';
            } else {
                secondarySeverityStyling = 'bg-white cursor-not-allowed text-blue-800 text-opacity-50 border-blue-800 border-opacity-50';
            }
        }
        ...
        if (props.severity === 'ghost') {
            if (!context.disabled) {
                ...
                ghostSeverityStyling = 'cursor-not-allowed text-gray-800 text-opacity-50 border-transparent';
            }
        }
