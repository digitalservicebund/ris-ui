...
export default {
  root: ({
    ...
    attrs: InputTextAttrs;
    parent: InputTextParent;
  }) => {
    ...
    const focusStyling =
      !isReadOnly && !isPartOfInputGroup
        ? `focus:outline-none focus:outline-blue-800 focus:outline-4 ${props.invalid ? "focus:text-black" : ""}`
        : "";
    const hoverStyling =
      !context.disabled && !isReadOnly ? "hover:bg-blue-200" : "";
    const hoverFocusStyling =
      !isReadOnly && !isPartOfInputGroup
        ? `focus:bg-white ${hoverStyling}`
        : "";
    const parentStyling = isPartOfInputGroup ? "outline-none" : "";
    const outlineStyling = !isPartOfInputGroup
      ? "outline outline-2 outline-solid outline-blue-800"
      : "";
