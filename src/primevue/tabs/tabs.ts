...
export const tab: TabPassThroughOptions = {
  root: ({ context }) => {
    ...
    const active = tw`bg-(--p-tabs-tab-active-background) text-black shadow-[-1px_-1px_0_0_var(--p-tabs-tab-border-color),1px_-1px_0_0_var(--p-tabs-tab-border-color)]`;
...
export const tabPanel: TabPanelPassThroughOptions = {
  root: {
    class: tw`min-h-96 bg-(--p-tabs-tabpanel-background) py-24 outline-blue-800 focus-visible:outline-solid`,
  },
};
export const tabList: TabListPassThroughOptions = {
  tabList: {
    class: tw`relative flex before:absolute before:bottom-0 before:left-[50%] before:h-px before:w-(--tab-list-separator-width,100%) before:-translate-x-1/2 before:bg-(--p-tabs-tablist-border-color)`,
  },
};
